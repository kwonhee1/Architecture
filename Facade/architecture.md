application architecture (기존 facade architecture와는 목적이 다릅니다)
...
output adapter (repository, ai service, redis, other api)
...
1. domain service는 다른 domain의 service 호출 불가, facade service는 다른 facade service 호출 불가 (공통 로직이 있다면 그것은 domain 로직일수 있고, domain service로 분리하는 것이 맞다)
...
외부 service의 책임은 facade service에 있습니다 (domain service 는 외부 adapter에 직접적으로 의존하지 않기 위함)

단, db와 같이 domain 로직에 밀접하게 관련있거나 domain 로직과 완벽히 분리가 어려운 경우 domain service에서 직접 호출합니다 (or port를 사용합니다, … 등등)
